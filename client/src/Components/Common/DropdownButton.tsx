...
export default function DropdownButton({ title, children }: React.PropsWithChildren<DropdownButtonProps>) {
  return <Menu as="div" className="relative">
    <Menu.Button className="inline-flex justify-center items-center bg-primary-800 rounded-md p-1 px-2 w-max">
...
Dropdown.Item = function ({ children, ...props }: React.PropsWithChildren<DropdownItemProps>) {
  return <Menu.Item as="button" {...props} className="w-full hover:bg-primary-700 hover:text-white flex text-secondary-900 px-2 py-2 text-sm">{children}</Menu.Item>
}